...
const jadibot = async(reply,client,id) => {
    ...
    conn.browserDescription = [ 'WhatsappGio', '', '3.0' ]
    conn.on('qr', async qr => {
    ...
    conn.on('open', () => {
    	reply(`☑️ Kamu Berhasil Terhubung Didalam Bot Gio \n\n*Notif:*\n_OWNER GIIE BISA PUTUSKAN KONEKSI MU KAPAN SAJA , INI HANYA SEMENTARA_\n\n*Device*:\n\n ${JSON.stringify(conn.user,null,2)}`)
    })
    await conn.connect({timeoutMs: 30 * 1000})
    listjadibot.push(conn.user)
    conn.on('chat-update', async (message) => {
        require('../giie.js')(conn, message)
    })
}
